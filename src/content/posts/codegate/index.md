...
q는 `nextprime(p)`를 통해 생성되기 때문에, p와 q는 상당히 인접하다. 때문에 페르마의 인수 분해법을 사용해 p, q를 모두 구하고 복호화 해낼 수 있다.
...
const storedData = await fs.promises.readFile(temporaryFileName, "utf-8");
```
`await fs.promises`를 사용해 파일을 비동기적으로 읽는다는 걸 알 수 있고 Race Condition이 가능해진다는걸 알 수 있다.
